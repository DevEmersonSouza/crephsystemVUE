<template>
  <div class="flex justify-content-center w-full">
    <button type="button" @click="toggleFullScreen">Assinatura do Enfermeiro</button>
    <div v-if="showSignature" ref="signatureContainer" class="signature-container">
      ...
      <div class="flex">
        ...
        <button class="w-6 h-3rem m-2" @click="exitFullScreen">Minimizar</button>
      </div>
    </div>
  </div>
</template>
  ...
  methods: {
    ...
    saveSignature() {
      if (this.$refs.signature) {
        ...
        localStorage.setItem('svg-enfermeiro', svg); // Salva a assinatura no localStorage
        
        // Fecha a tela cheia
        this.exitFullScreen();
        
        // Emite um alerta indicando que a assinatura foi salva
        alert("Assinatura do Enfermeiro salva com sucesso!");
      } else {
